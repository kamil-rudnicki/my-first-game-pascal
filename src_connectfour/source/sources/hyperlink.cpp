...
bool wxHyperLink::Create (wxWindow *parent,
    ...
    m_HoverColour = wxColour (_("RED"));
    m_NormalColour = wxColour (_("BLUE"));
    m_VisitedColour = wxColour (_("PURPLE"));
...
void wxHyperLink::ExecuteLink (const wxString &link) {
    ...
    if (link.StartsWith (_("http://"))) {
        mimetype = _("text/html");
    }else if (link.StartsWith (_("ftp://"))) {
        mimetype = _("text/html");
    }else if (link.StartsWith (_("mailto:"))) {
        mimetype = _("message/rfc822");
    }else{
        return;
    }
    ...
    if (filetype) {
        ...
        if (filetype->GetOpenCommand (&cmd, wxFileType::MessageParameters (link))) {
            cmd.Replace(_("file://"), wxEmptyString);
            ::wxExecute(cmd);
        }
        delete filetype;
    }
